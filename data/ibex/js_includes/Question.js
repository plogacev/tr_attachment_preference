  ...
  define_ibex_controller({
    ...
    jqueryWidget: {
      _init: function () {
        ...
        if (this.hasCorrect === true) this.hasCorrect = 0;
        if (typeof this.hasCorrect == "string") {
          var foundIt = false;
          for (var i = 0; i < this.answers.length; ++i) {
            if (
              this.answers[i].toLowerCase() == this.hasCorrect.toLowerCase()
            ) {
              this.hasCorrect = i;
              foundIt = true;
              break;
          ...
          assert(
            foundIt,
            "Value of 'hasCorrect' option not recognized in Question"
          );
        }
        this.showNumbers = dget(this.options, "showNumbers", true);
        this.presentAsScale = dget(this.options, "presentAsScale", false);
        this.randomOrder = dget(
          this.options,
          "randomOrder",
          !(this.hasCorrect === false || this.presentAsScale)
        );
        ...
        if (!(this.hasCorrect === false))
          assert(
            typeof this.hasCorrect == "number" &&
              this.hasCorrect < this.answers.length,
            "Bad index for correct answer in Question"
          );

        if (this.randomOrder) {
          assert(
            typeof this.answers[0] != "object",
            "Cannot set 'randomOrder' option to a list of keys when keys are included with the 'as' option."
          );
          assert(
            typeof this.answers[0] != "object" ||
              this.answers.length == this.randomOrder.length,
        ...
        if (
          (this.presentAsScale || this.presentHorizontally) &&
          this.leftComment
        ) {
        ...
        for (var i = 0; i < this.orderedAnswers.length; ++i) {
          ...
          if (this.presentAsScale || this.presentHorizontally) {
            ...
            (function (li) {
              li.mouseover(function () {
                li.css("border-color", "black").css("cursor", "pointer");
              });
              li.mouseout(function () {
                li.css("border-color", "#9ea4b1").css("cursor", "default");
              });
            })(li);
          } else {
          ...
          var ans =
            typeof this.orderedAnswers[i] == "string"
              ? this.orderedAnswers[i]
              : this.orderedAnswers[i][1];
          var t = this; // 'this' doesn't behave as a lexically scoped variable so can't be
          // captured in the closure defined below.
          var a = $(document.createElement("span")).addClass(
            this.cssPrefix + "fake-link"
          );
          ...
          __Question_callback__ = function (i) {
            ...
            if (!(t.hasCorrect === false)) {
              var correct_ans =
                typeof t.answers[t.hasCorrect] == "string"
                  ? t.answers[t.hasCorrect]
                  : t.answers[t.hasCorrect][1];
            ...
            t.finishedCallback([
              [
                [
                  questionField,
                  t.question ? csv_url_encode(t.question) : "NULL",
                ],
        ...
        if (!(this.qp === undefined)) this.element.append(this.qp);

        // Again, using tables to center because IE sucks.
        var table = $(
          "<table" + (conf_centerItems ? " align='center'" : "") + ">"
        );
        var tr = $(document.createElement("tr"));
        var td = $("<td" + (conf_centerItems ? " align='center'" : "") + ">");
        if (conf_centerItems) td.attr("align", "center");
        ...
        this.safeBind($(document), "keydown", function (e) {
          ...
          if (
            ...
            if (n > 0 && n <= t.orderedAnswers.length) {
              ...
              var ans =
                typeof t.orderedAnswers[n - 1] == "string"
                  ? t.orderedAnswers[n - 1]
                  : t.orderedAnswers[n - 1][1];
              var correct = "NULL";
              if (!(t.hasCorrect === false)) {
                var correct_ans =
                  typeof t.answers[t.hasCorrect] == "string"
          ...
          // Letters (and numbers in the case of scales).
            ...
            console.log(code);
            console.log(t.orderedAnswers);

            var ans = null;
            console.log(t.randomOrder);

            if (typeof t.randomOrder != "object") {
              for (var i = 0; i < t.answers.length; ++i) {
                if (
                  t.autoFirstChar &&
                  typeof t.answers[i] == "string" &&
                  code == t.answers[i].toUpperCase().charCodeAt(0)
                ) {
                  ans = t.answers[i];
                  break;
                } else if (
                  $.isArray(t.answers[i]) &&
                  code == t.answers[i][0].toUpperCase().charCodeAt(0)
                ) {
                  ans = t.answers[i][1];
                  break;
                }
